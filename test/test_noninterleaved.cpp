...
{
    EXPECT_EQ(
        static_cast<std::size_t>(std::distance(this->container_.begin(), this->container_.end())), this->channels());
}
...
protected:
    ...
    static ratl::noninterleaved<sample_value_type> generate_default()
    {
        return ratl::noninterleaved<sample_value_type>();
    }

    static ratl::noninterleaved<sample_value_type> generate_typical()
    {
        return ratl::noninterleaved<sample_value_type>(TEST_RATL_TYPICAL_CHANNELS, TEST_RATL_TYPICAL_FRAMES);
    }
};
...
{
    EXPECT_EQ(this->generate_default(), (ratl::noninterleaved<typename TestFixture::sample_value_type>(0, 0)));
}
...
TYPED_TEST(Equality, DefaultZeroChannelsConstructorNotEqual)
{
    EXPECT_NE(
        ...
        (ratl::noninterleaved<typename TestFixture::sample_value_type>(0, TEST_RATL_TYPICAL_FRAMES)));
}
...
TYPED_TEST(Equality, DefaultZeroFramesConstructorNotEqual)
{
    EXPECT_NE(
        this->generate_default(),
        (ratl::noninterleaved<typename TestFixture::sample_value_type>(TEST_RATL_TYPICAL_CHANNELS, 0)));
}
