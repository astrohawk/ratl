...
PYBIND11_MODULE(ratl_pybind, m)
{
    ...
    m.def(
        ...
        []()
        {
            std::vector<double> samples(FrameSize / 2);
            double scale = static_cast<double>(SampleRate) / static_cast<double>(FrameSize);
            for (std::size_t i = 0; i < samples.size(); ++i)
            {
                samples[i] = static_cast<double>(i) * scale;
            }
            return samples;
        });
    m.def(
        ...
        []()
        {
            ...
            Gist<double> gist(static_cast<int>(FrameSize), static_cast<int>(SampleRate));
            gist.processAudioFrame(samples);
            return gist.getMagnitudeSpectrumDb();
        });
    m.def(
        "reference_int_sine",
        []()
    ...
    m.def(
        ...
        []()
        {
            ...
            Gist<float> gist(static_cast<int>(FrameSize), static_cast<int>(SampleRate));
            gist.processAudioFrame(samples);
            return gist.getMagnitudeSpectrumDb();
        });
}